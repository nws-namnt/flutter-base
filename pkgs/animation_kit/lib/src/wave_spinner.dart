...
/// A circular track with a half-circle arc that spins around it (eased by
/// [curve]) plus an optional animated sine wave rippling inside the track
/// (only drawn when [child] is null); an optional [child] can be centered
/// inside the spinner instead of the wave.
class WaveSpinner extends StatefulWidget {
  ...
  /// Color of the static background track. Defaults to a translucent grey
  /// (`0x68757575`).
  final Color trackColor;

  /// Color of the rippling sine wave, drawn only when [child] is null.
  /// Defaults to a translucent grey (`0x68757575`).
  final Color waveColor;

  /// Side length of the square area the spinner occupies. Defaults to `50`.
  final double size;

  /// Duration of one full arc rotation / wave cycle. Defaults to `3000`
  /// milliseconds.
  final Duration duration;

  /// Easing curve applied to the arc rotation and wave animation. Defaults
  /// to [Curves.decelerate].
  final Curve curve;

  /// Optional widget centered inside the spinner. When provided, the
  /// rippling wave is not drawn.
  final Widget? child;
...
class _WaveSpinnerState extends State<WaveSpinner> with SingleTickerProviderStateMixin {
...
/// Paints the [WaveSpinner] track, the rotating arc, and (when there is no
/// child) an animated sine wave rippling vertically inside the track.
class WaveCustomPaint extends CustomPainter {
  WaveCustomPaint({
    required this.color,
    required this.trackColor,
    required this.waveColor,
    /// Drives the arc rotation and wave animations; also used as the
    /// [CustomPainter] repaint listenable.
    required AnimationController controller,
    /// Whether a child widget is centered in the spinner; when true the
    /// rippling wave is not drawn.
    required bool hasChild,
    /// Easing curve applied to the arc rotation and wave animations.
    required Curve curve,
    /// Size of the area being painted, used to compute the wave's maximum
    /// radius.
    required Size size,
  ...
  /// Color of the static background track.
  final Color trackColor;

  /// Color of the rippling sine wave.
  final Color waveColor;
