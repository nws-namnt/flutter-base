...
/// A set of concentric, capsule-shaped lines that each rotate at a
/// different speed (scaled by their distance from the center), creating a
/// nested spinning-lines loader effect.
class SpinningLines extends StatefulWidget {
  ...
  /// Color used to draw each line.
  final Color color;

  /// Side length of the square area the animation is painted in. Defaults to `70`.
  final double size;

  /// Stroke width of each line. Defaults to `2.0`.
  final double lineWidth;

  /// Number of nested lines to draw, each rotating at a different speed.
  final int itemCount;

  /// Duration of one full rotation cycle. Defaults to `3000` milliseconds.
  ...
  @override
  State<SpinningLines> createState() => _SpinningLinesState();
}
...
/// Paints the set of nested, rotating capsule-shaped lines for
/// [SpinningLines], one per item, each scaled and rotated at a speed
/// proportional to its distance from the center.
class SpinningLinesPainter extends CustomPainter {
  SpinningLinesPainter(
    /// Current animation progress, from `0.0` to `1.0`, driving the rotation
    /// of each line.
    this.rotateValue, {
  ...
  /// Number of nested lines to draw.
  final int itemCount;
